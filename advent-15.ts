...
const puzzleInput15 = stdinBuffer.toString();
...
function createVertices(riskMap: Array<Array<number>>): Array<Vertex> {
    let vertexIndex = 0;
...
function dijkstrasShortestPath(startIndex: number, destionationIndex: number, vertices: Array<Vertex>): number {
    const lowestRiskFromStart: Array<number> = Array(vertices.length).fill(Number.POSITIVE_INFINITY);
    lowestRiskFromStart[startIndex] = 0;
    ...
    return lowestRiskFromStart[destionationIndex];
}

function partOne15(): void {
    ...
    const vertices = createVertices(riskMap);
    const lowestRiskFromStartToDestionation = dijkstrasShortestPath(0, vertices.length-1, vertices)
    console.log("Lowest risk path in Part 1: ", lowestRiskFromStartToDestionation);
}

function partTwo15(): void {
    const riskMap: Array<Array<number>> = [];

    for(let rowRepeat = 0; rowRepeat < 5; rowRepeat++) {
        for (let row = 0; row < puzzleInputArray15.length; row++) {
            let numberRow = [];
            for(let columnRepeat = 0; columnRepeat < 5; columnRepeat++) {
                for(let column = 0; column < puzzleInputArray15[row].length; column++) {
                    let risk = parseInt(puzzleInputArray15[row][column]) + rowRepeat + columnRepeat;
                    numberRow.push(risk <= 9 ? risk: risk % 9);
    ...
    const vertices: Array<Vertex> = createVertices(riskMap);;
    const lowestRiskFromStartToDestionation = dijkstrasShortestPath(0, vertices.length-1, vertices)
    console.log("Lowest risk path in Part 2: ", lowestRiskFromStartToDestionation);
}
