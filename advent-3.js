...
function partOne() {
    ...
    let bitCountArray = new Array(puzzleInputArray[0].length).fill(0);

    for(let i = 0; i < puzzleInputArray.length; i++) {
        for(let j = 0; j < puzzleInputArray[i].length; j++) {
            if (parseInt(puzzleInputArray[i][j]) === 1) {
                bitCountArray[j] += 1;
            } 
            else {
                bitCountArray[j] -= 1;
    ...
    const gammaBinaryString = gammaArray.join("");
    const gamma = parseInt(gammaBinaryString, 2);

    const epsilonBinaryString =  epsilonArray.join("");
